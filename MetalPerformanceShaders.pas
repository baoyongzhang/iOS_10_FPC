...
unit MetalPerformanceShaders;
interface

{$linkframework MetalPerformanceShaders}

uses
{$if defined(IPHONEALL) and not defined(GROUP_INDEPENDENT)}
	 DefinedClassesMetalPerformanceShaders, Metal, CTypes, MacTypes, MacOSXPosix, iPhoneAll;
{$else}
	 DefinedClassesMetalPerformanceShaders, CoreGraphics, Metal, Foundation, CTypes, MacTypes, MacOSXPosix;
{$endif}
...
{$include metalperformanceshaders/UndefinedTypes.inc}

{$define TYPES}
{$include metalperformanceshaders/Sources.inc}
{$undef TYPES}
...
{$include metalperformanceshaders/InlineFunctions.inc}
