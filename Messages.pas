...
{$linkframework Messages}

uses
{$if defined(IPHONEALL) and not defined(GROUP_INDEPENDENT)}
	 DefinedClassesMessages, CTypes, MacTypes, MacOSXPosix, iPhoneAll;
{$else}
	 DefinedClassesMessages, Foundation, UIKit, CTypes, MacTypes, MacOSXPosix;
{$endif}
...
{$include messages/UndefinedTypes.inc}

{$define TYPES}
{$include messages/Sources.inc}
{$undef TYPES}
...
{$include messages/InlineFunctions.inc}
